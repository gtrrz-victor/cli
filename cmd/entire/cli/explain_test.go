...
func TestRunExplainAuto_CommitRefWithCheckpointTrailer(t *testing.T) {
	...
	require.NoError(t, checkpoint.NewGitStore(repo, checkpoint.DefaultV1Refs()).WriteCommitted(ctx, checkpoint.WriteOptions{
		CheckpointID: cpID,
...
func writeTemporaryCheckpointForExplainTest(t *testing.T) string {
	...
	result, err := checkpoint.NewEphemeralStore(repo, checkpoint.DefaultV1Refs()).WriteTemporary(context.Background(), checkpoint.WriteTemporaryOptions{
		SessionID:         sessionID,
...
func TestRunExplainCheckpoint_AmbiguousCommittedPrefixPrintsToErrWAndReturnsSilent(t *testing.T) {
	...
	for _, cpID := range []id.CheckpointID{
		...
		require.NoError(t, store.WriteCommitted(ctx, checkpoint.WriteOptions{
			CheckpointID: cpID,
...
func TestRunExplainCheckpoint_V1PreservesTranscriptOffset(t *testing.T) {
	...
	require.NoError(t, checkpoint.NewGitStore(repo, checkpoint.DefaultV1Refs()).WriteCommitted(context.Background(), checkpoint.WriteOptions{
		CheckpointID:              cpID,
...
func TestRunExplainCheckpoint_GenerateWritesV1Store(t *testing.T) {
	...
	require.NoError(t, v1Store.WriteCommitted(ctx, checkpoint.WriteOptions{
		CheckpointID: cpID,
		SessionID:    "session-v1",
		Strategy:     "manual-commit",
...
func TestGetBranchCheckpoints_DefaultBranchFindsMergedCheckpoints(t *testing.T) {
	...
	store := checkpoint.NewGitStore(repo, checkpoint.DefaultV1Refs())
	if err := store.WriteCommitted(context.Background(), checkpoint.WriteOptions{
		CheckpointID: cpID,
