...
func writeTemporaryCheckpointForExplainTest(t *testing.T) string {
	...
	result, err := checkpoint.NewEphemeralStore(repo, checkpoint.DefaultV1Refs()).WriteTemporary(context.Background(), checkpoint.WriteEphemeralOptions{
		SessionID:         sessionID,
...
func TestGetBranchCheckpoints_ReadsPromptFromShadowBranch(t *testing.T) {
	...
	baseCommit := initialCommit.String()[:7]
	_, err = store.WriteTemporary(context.Background(), checkpoint.WriteEphemeralOptions{
		SessionID:         sessionID,
...
func TestGetReachableTemporaryCheckpoints_FiltersByWorktree(t *testing.T) {
	...
	writeCheckpoints := func(sessionID, worktreeID string) {
		...
		// Baseline
		if _, err := store.WriteTemporary(context.Background(), checkpoint.WriteEphemeralOptions{
